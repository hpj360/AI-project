---
id: ENT-iba-mimosa
title: 含羞草
category: ENT
subcategory: cocktail
tags: [鸡尾酒, IBA, 当代经典（Contemporary Classics）, Mimosa, 摇和]
status: active
created: 2026-07-11
updated: 2026-07-11
related: [ENT-cocktail-chinese-colada, ENT-cocktail-jasmine-tea-cocktail, ENT-cocktail-yuzu-margarita, ENT-cocktail-lychee-martini, ENT-cocktail-negroni]
related_typed: {ENT-cocktail-chinese-colada: same_subcat, ENT-cocktail-jasmine-tea-cocktail: same_subcat, ENT-cocktail-yuzu-margarita: same_subcat, ENT-cocktail-lychee-martini: same_subcat, ENT-cocktail-negroni: same_subcat}
data_confidence: official
data_source: IBA Official
version: 1
ratings:
  vivino: {score: 4.4, votes: 21460}
  diffords: {score: 4.12, year: 2020}
  iba: {score: 4.1, year: 2021}
source_url: https://iba-world.com/iba-cocktail/mimosa/
crawl_date: 2024-07-07
version: 1
---

# 含羞草

**Mimosa**

## 概述

IBA 官方 当代经典（Contemporary Classics） 系列鸡尾酒 Mimosa（含羞草），以 Freshly Squeezed Orange Juice、Prosecco 调制，摇和入鸡尾酒杯。

## 图片参考

- [WikiMedia Commons](https://commons.wikimedia.org/w/index.php?search=Mimosa%20cocktail%20bottle&title=Special:MediaSearch&type=image)
- [Unsplash 图库](https://unsplash.com/s/photos/Mimosa%20cocktail)
- [Google 图片](https://www.google.com/search?q=Mimosa%20cocktail%20bottle&tbm=isch)

## 基础信息

- **中文名**：含羞草
- **外文名**：Mimosa
- **分类**：鸡尾酒
- **产地**：
- **酒精度**：
- **参考价格（RMB）**：¥50-200
- **价格档位**：

## 创制信息

- **IBA 分类**：contemporary_classics
- **风格分类**：iba_classic

## 生产工艺

- **原料**：Freshly Squeezed Orange Juice、Prosecco
- **酿造方法**：

-external, google_font-enabled, font_display-swap"> .e-con.e-parent:nth-of-type(n+4):not(.e-lazyloaded):not(.e-no-lazyload), .e-con.e-parent:nth-of-type(n+4):not(.e-lazyloaded):not(.e-no-lazyload) * { background-image: none !important; } @media screen and (max-height: 1024px) { .e-con.e-parent:nth-of-type(n+3):not(.e-lazyloaded):not(.e-no-lazyload), .e-con.e-parent:nth-of-type(n+3):not(.e-lazyloaded):not(.e-no-lazyload) * { background-image: none !important; } } @media screen and (max-height: 640px) { .e-con.e-parent:nth-of-type(n+2):not(.e-lazyloaded):not(.e-no-lazyload), .e-con.e-parent:nth-of-type(n+2):not(.e-lazyloaded):not(.e-no-lazyload) * { background-image: none !important; } } IBA Menu IBA About Us Constitution Board Academy IBA WCC Live Stream Guilds Cocktails All Cocktails The Unforgettables The Contemporary The New Era Supporters Events Contact Us User Shopping-bag Facebook-f Linkedin-in Instagram Youtube

## 配方

| 材料 | 用量 | 单位 |
|------|------|------|
| Freshly Squeezed Orange Juice | 75 | ml |
| Prosecco | 75 | ml |

- **调制技法**：摇和
- **难度**：★★☆☆☆ 简单
- **装饰**：Garnish with orange twist (optional).
- **用冰类型**：方块冰
- **风味平衡**：均衡
- **适饮季节**：四季
- **适饮场合**：餐前

## 风味轮廓

| 维度 | 评分 (1-5) | 轮廓 |
|------|-----------|------|
| 甜 | 3 | ███░░ |
| 酸 | 3 | ███░░ |
| 苦 | 2 | ██░░░ |
| 烈 | 3 | ███░░ |
| 香 | 3 | ███░░ |

## 评分奖项

| 评分机构 | 评分 | 年份/票数 |
|---------|------|----------|
| Vivino 用户 | 4.4 | 21460 |
| Difford's Guide | 4.12 | 2020 |
| IBA 推荐 | 4.1 | 2021 |

> 注：以上为参考评分（离线知识库整理），实际以官方发布为准。

## 饮用指南

- **适饮温度**：冰镇
- **推荐酒杯**：鸡尾酒杯
- **佐餐搭配**：小食
- **鸡尾酒应用**：餐前
- **饮用建议**：将橙汁倒入冰镇笛型杯，缓慢注入冰镇香槟。

## 合规信息

- **可购性**：酒吧
- **合规提示**：理性饮酒，未成年人禁止饮酒

## 参考资料

- 本条目由 content-creation 知识库构建系统生成，基于公开资料整理。
- 数据来源：IBA Official
- 数据置信度：official
