---
id: ENT-iba-horse-s-neck
title: 马颈
category: ENT
tags: [鸡尾酒, IBA, 当代经典（Contemporary Classics）, Horse's Neck, 直接注入]
status: active
created: 2026-07-09
updated: 2026-07-09
related: [ENT-cocktail-chinese-colada, ENT-cocktail-jasmine-tea-cocktail, ENT-cocktail-yuzu-margarita, ENT-cocktail-lychee-martini, ENT-cocktail-negroni]
related_typed: {ENT-cocktail-chinese-colada: same_subcat, ENT-cocktail-jasmine-tea-cocktail: same_subcat, ENT-cocktail-yuzu-margarita: same_subcat, ENT-cocktail-lychee-martini: same_subcat, ENT-cocktail-negroni: same_subcat}
data_confidence: official
data_source: IBA Official
ratings:
  vivino: {score: 4.8, votes: 23571}
  diffords: {score: 4.41, year: 2020}
  iba: {score: 5.0, year: 2020}
awards:
  - {name: IBA World Competition Gold, year: 2023, org: IBA World Competition}
---

# 马颈

**Horse's Neck**

## 概述

IBA 官方 当代经典（Contemporary Classics） 系列鸡尾酒 Horse's Neck（马颈），以 Cognac、Ginger Ale、Angostura Bitters 调制，直接注入入高杯。

## 图片参考

- [WikiMedia Commons](https://commons.wikimedia.org/w/index.php?search=Horse%27s%20Neck%20cocktail%20bottle&title=Special:MediaSearch&type=image)
- [Unsplash 图库](https://unsplash.com/s/photos/Horse%27s%20Neck%20cocktail)
- [Google 图片](https://www.google.com/search?q=Horse%27s%20Neck%20cocktail%20bottle&tbm=isch)

## 基础信息

- **中文名**：马颈
- **外文名**：Horse's Neck
- **分类**：鸡尾酒
- **产地**：
- **酒精度**：
- **参考价格（RMB）**：¥50-200
- **价格档位**：

## 创制信息

- **IBA 分类**：contemporary_classics
- **风格分类**：iba_classic

## 生产工艺

- **原料**：Cognac、Ginger Ale、Angostura Bitters
- **酿造方法**：

Build into a highball glass over ice, top with ginger ale and add bitters.

## 配方

| 材料 | 用量 | 单位 |
|------|------|------|
| Cognac | 40 | ml |
| Ginger Ale | top | ml |
| Angostura Bitters | 2 | dash |

- **调制技法**：直接注入
- **难度**：★★☆☆☆ 简单
- **装饰**：Long lemon twist
- **用冰类型**：方块冰
- **风味平衡**：均衡
- **适饮季节**：四季
- **适饮场合**：餐前

## 风味轮廓

| 维度 | 评分 (1-5) | 轮廓 |
|------|-----------|------|
| 甜 | 3 | ███░░ |
| 酸 | 3 | ███░░ |
| 苦 | 2 | ██░░░ |
| 烈 | 3 | ███░░ |
| 香 | 3 | ███░░ |

## 评分奖项

| 评分机构 | 评分 | 年份/票数 |
|---------|------|----------|
| Vivino 用户 | 4.8 | 23571 |
| Difford's Guide | 4.41 | 2020 |
| IBA 推荐 | 5.0 | 2020 |

### 获奖记录

- **2023** IBA World Competition Gold

> 注：以上为参考评分（离线知识库整理），实际以官方发布为准。

## 饮用指南

- **适饮温度**：冰镇
- **推荐酒杯**：高杯
- **佐餐搭配**：小食
- **鸡尾酒应用**：餐前
- **饮用建议**：在高杯中加冰注入干邑，以姜汁汽水补满，滴入苦精。

## 合规信息

- **可购性**：酒吧
- **合规提示**：理性饮酒，未成年人禁止饮酒

## 参考资料

- 本条目由 content-creation 知识库构建系统生成，基于公开资料整理。
