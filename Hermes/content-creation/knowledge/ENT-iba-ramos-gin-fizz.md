---
id: ENT-iba-ramos-gin-fizz
title: 拉莫斯金菲士
category: ENT
tags: [鸡尾酒, IBA, 难忘杯（The Unforgettables）, Ramos Gin Fizz, 摇和]
status: active
created: 2026-07-09
updated: 2026-07-09
related: [ENT-cocktail-baijiu-sour, ENT-cocktail-maotai-martini, ENT-cocktail-chinese-colada, ENT-cocktail-earl-grey-martini, ENT-cocktail-jasmine-tea-cocktail]
related_typed: {ENT-cocktail-baijiu-sour: same_subcat, ENT-cocktail-maotai-martini: same_subcat, ENT-cocktail-chinese-colada: same_subcat, ENT-cocktail-earl-grey-martini: same_subcat, ENT-cocktail-jasmine-tea-cocktail: same_subcat}
data_confidence: official
data_source: IBA Official
ratings:
  vivino: {score: 4.8, votes: 5152}
  diffords: {score: 4.27, year: 2022}
  iba: {score: 4.8, year: 2021}
---

# 拉莫斯金菲士

**Ramos Gin Fizz**

## 概述

IBA 官方 难忘杯（The Unforgettables） 系列鸡尾酒 Ramos Gin Fizz（拉莫斯金菲士），以 Gin、Lemon Juice、Lime Juice、Simple Syrup、Cream、Egg White、Soda Water、Orange Flower Water 调制，摇和入高杯。

## 图片参考

- [WikiMedia Commons](https://commons.wikimedia.org/w/index.php?search=Ramos%20Gin%20Fizz%20cocktail%20bottle&title=Special:MediaSearch&type=image)
- [Unsplash 图库](https://unsplash.com/s/photos/Ramos%20Gin%20Fizz%20cocktail)
- [Google 图片](https://www.google.com/search?q=Ramos%20Gin%20Fizz%20cocktail%20bottle&tbm=isch)

## 基础信息

- **中文名**：拉莫斯金菲士
- **外文名**：Ramos Gin Fizz
- **分类**：鸡尾酒
- **产地**：
- **酒精度**：
- **参考价格（RMB）**：¥50-200
- **价格档位**：

## 创制信息

- **IBA 分类**：unforgettables
- **风格分类**：iba_classic

## 生产工艺

- **原料**：Gin、Lemon Juice、Lime Juice、Simple Syrup、Cream、Egg White、Soda Water、Orange Flower Water
- **酿造方法**：

Shake all ingredients (except soda) vigorously with ice, strain into a highball glass and top with soda water.

## 配方

| 材料 | 用量 | 单位 |
|------|------|------|
| Gin | 45 | ml |
| Lemon Juice | 15 | ml |
| Lime Juice | 15 | ml |
| Simple Syrup | 30 | ml |
| Cream | 60 | ml |
| Egg White | 1 | 个 |
| Soda Water | top | ml |
| Orange Flower Water | 2 | dash |

- **调制技法**：摇和
- **难度**：★★☆☆☆ 简单
- **装饰**：None
- **用冰类型**：方块冰
- **风味平衡**：均衡
- **适饮季节**：四季
- **适饮场合**：餐前

## 风味轮廓

| 维度 | 评分 (1-5) | 轮廓 |
|------|-----------|------|
| 甜 | 3 | ███░░ |
| 酸 | 3 | ███░░ |
| 苦 | 2 | ██░░░ |
| 烈 | 3 | ███░░ |
| 香 | 3 | ███░░ |

## 评分奖项

| 评分机构 | 评分 | 年份/票数 |
|---------|------|----------|
| Vivino 用户 | 4.8 | 5152 |
| Difford's Guide | 4.27 | 2022 |
| IBA 推荐 | 4.8 | 2021 |

> 注：以上为参考评分（离线知识库整理），实际以官方发布为准。

## 饮用指南

- **适饮温度**：冰镇
- **推荐酒杯**：高杯
- **佐餐搭配**：小食
- **鸡尾酒应用**：餐前
- **饮用建议**：将所有材料（除苏打水）加冰用力摇匀，滤入高杯后以苏打水补满。

## 合规信息

- **可购性**：酒吧
- **合规提示**：理性饮酒，未成年人禁止饮酒

## 参考资料

- 本条目由 content-creation 知识库构建系统生成，基于公开资料整理。
