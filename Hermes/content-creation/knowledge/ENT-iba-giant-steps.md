---
id: ENT-iba-giant-steps
title: 巨步
category: ENT
subcategory: cocktail
tags: [鸡尾酒, IBA, 新时代（New Era Drinks）, Giant Steps, 摇和]
status: active
created: 2026-07-17
updated: 2026-07-17
related: [ENT-cocktail-americano, ENT-cocktail-margarita, ENT-cocktail-daiquiri, ENT-cocktail-sidecar, ENT-cocktail-sazerac]
related_typed: {ENT-cocktail-americano: same_subcat, ENT-cocktail-margarita: same_subcat, ENT-cocktail-daiquiri: same_subcat, ENT-cocktail-sidecar: same_subcat, ENT-cocktail-sazerac: same_subcat}
data_confidence: official
data_source: IBA Official
version: 1
---

# 巨步

**Giant Steps**

## 概述

IBA 官方 新时代（New Era Drinks） 系列鸡尾酒 Giant Steps（巨步），以 Gin、Lillet Blanc、Lemon Juice、Apricot Brandy Liqueur 调制，摇和入鸡尾酒杯。

## 图片参考

- [WikiMedia Commons](https://commons.wikimedia.org/w/index.php?search=Giant%20Steps%20cocktail%20bottle&title=Special:MediaSearch&type=image)
- [Unsplash 图库](https://unsplash.com/s/photos/Giant%20Steps%20cocktail)
- [Google 图片](https://www.google.com/search?q=Giant%20Steps%20cocktail%20bottle&tbm=isch)

## 基础信息

- **中文名**：巨步
- **外文名**：Giant Steps
- **分类**：鸡尾酒
- **产地**：
- **酒精度**：
- **参考价格（RMB）**：¥50-200
- **价格档位**：

## 创制信息

- **IBA 分类**：new_era
- **风格分类**：iba_classic

## 生产工艺

- **原料**：Gin、Lillet Blanc、Lemon Juice、Apricot Brandy Liqueur
- **酿造方法**：

Pour all ingredients into cocktail shaker filled with ice cubes. Shake and strain into a chilled cocktail glass.

## 配方

| 材料 | 用量 | 单位 |
|------|------|------|
| Gin | 45 | ml |
| Lillet Blanc | 20 | ml |
| Lemon Juice | 15 | ml |
| Apricot Brandy Liqueur | 10 | ml |

- **调制技法**：摇和
- **难度**：★★☆☆☆ 简单
- **装饰**：Lemon twist
- **用冰类型**：方块冰
- **风味平衡**：均衡
- **适饮季节**：四季
- **适饮场合**：餐前

## 风味轮廓

| 维度 | 评分 (1-5) | 轮廓 |
|------|-----------|------|
| 甜 | 3 | ███░░ |
| 酸 | 3 | ███░░ |
| 苦 | 2 | ██░░░ |
| 烈 | 3 | ███░░ |
| 香 | 3 | ███░░ |

## 饮用指南

- **适饮温度**：冰镇
- **推荐酒杯**：鸡尾酒杯
- **佐餐搭配**：小食
- **鸡尾酒应用**：餐前
- **饮用建议**：将所有材料倒入加满冰块的摇酒壶，摇匀后滤入冰镇鸡尾酒杯。

## 合规信息

- **可购性**：酒吧
- **合规提示**：理性饮酒，未成年人禁止饮酒

## 参考资料

- 本条目由 content-creation 知识库构建系统生成，基于公开资料整理。
- 数据来源：IBA Official
- 数据置信度：official
