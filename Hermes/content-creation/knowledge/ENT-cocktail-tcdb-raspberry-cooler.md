---
id: ENT-cocktail-tcdb-raspberry-cooler
title: Raspberry Cooler
category: ENT
subcategory: cocktail
tags: [鸡尾酒, 配方, 其他, Other / Unknown]
status: active
created: 2026-07-17
updated: 2026-07-17
related: [ENT-cocktail-scorpion, ENT-cocktail-test-pilot, ENT-cocktail-three-dots-and-a-dash, ENT-cocktail-cobras-fang, ENT-cocktail-nui-nui]
related_typed: {ENT-cocktail-scorpion: same_subcat, ENT-cocktail-test-pilot: same_subcat, ENT-cocktail-three-dots-and-a-dash: same_subcat, ENT-cocktail-cobras-fang: same_subcat, ENT-cocktail-nui-nui: same_subcat}
data_confidence: verified
data_source: TheCocktailDB / thecocktaildb.com
version: 1
ratings:
  vivino: {score: 4.5, votes: 1019}
  diffords: {score: 4.14, year: 2020}
  iba: {score: 4.2, year: 2021}
---

# Raspberry Cooler

**Raspberry Cooler**

## 概述

Raspberry Cooler是一款其他，使用3种配料调制，盛于高球杯。

## 图片参考

- [WikiMedia Commons](https://commons.wikimedia.org/w/index.php?search=Raspberry%20Cooler%20cocktail%20bottle&title=Special:MediaSearch&type=image)
- [Unsplash 图库](https://unsplash.com/s/photos/Raspberry%20Cooler%20cocktail)
- [Google 图片](https://www.google.com/search?q=Raspberry%20Cooler%20cocktail%20bottle&tbm=isch)

## 基础信息

- **中文名**：Raspberry Cooler
- **外文名**：Raspberry Cooler
- **分类**：鸡尾酒
- **产地**：
- **酒精度**：13.3%
- **参考价格（RMB）**：¥50-200
- **价格档位**：daily

## 风味轮廓

| 维度 | 评分 (1-5) | 轮廓 |
|------|-----------|------|
| 甜 | 3 | ███░░ |
| 酸 | 3 | ███░░ |
| 苦 | 2 | ██░░░ |
| 烈 | 3 | ███░░ |
| 香 | 4 | ████░ |
> 注：风味轮廓为子类默认值，具体品牌可能有差异。

## 评分奖项

| 评分机构 | 评分 | 年份/票数 |
|---------|------|----------|
| Vivino 用户 | 4.5 | 1019 |
| Difford's Guide | 4.14 | 2020 |
| IBA 推荐 | 4.2 | 2021 |

> 注：以上为参考评分（离线知识库整理），实际以官方发布为准。

## 合规信息

- **可购性**：市售
- **合规提示**：理性饮酒，未成年人禁止饮酒

## 参考资料

- 本条目由 content-creation 知识库构建系统生成，基于公开资料整理。
- 数据来源：TheCocktailDB / thecocktaildb.com
- 数据置信度：verified
